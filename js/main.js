$(function () {
	...
	function is_adjust(n) {
		...
		if (n == "1") {
			t =
				blogName_width + search_width + mw > $("#page-header").width() - 300
					? true
					: false;
		} else if (n == "2") {
			t =
				blogName_width + search_width + mw > $("#page-header").width()
					? true
					: false;
		} else if (n == "3") {
			t =
	...
	if ($("#sidebar").hasClass("auto_open")) {
		...
		} else $("#toggle-sidebar").css("display", "none");
	} else {
	...
	if (highlight_lang == "true") {
		...
		$("figure").each(function () {
			...
			if (lang_name_index == "js") lang_name = "javascript";
			if (lang_name_index == "md") lang_name = "markdown";
			if (lang_name_index == "plain") lang_name = "code";
			if (lang_name_index == "py") lang_name = "python";
	...
	if (medium_zoom == "false") {
		...
		for (var i = 0; i < imgList.length; i++) {
			var lazyload_src = imgList[i].src
				? imgList[i].src
				: imgList.eq(i).attr("data-src");
		...
		const zoom = mediumZoom(
			document.querySelectorAll("#post img,.justified-gallery img")
		);
		zoom.on("open", (event) => {
			let photoBg =
	...
	if ($(".justified-gallery").length) {
		...
		$(
			".justified-gallery > p > .fancybox,.justified-gallery > p > div"
		).unwrap();
		$("head").append(
			'<link rel="stylesheet" type="text/css" href="https://cdn.bilicdn.tk/npm/justifiedGallery@3.7.0/dist/css/justifiedGallery.min.css">'
		);
		loadScript(
			"https://cdn.bilicdn.tk/npm/justifiedGallery@3.7.0/dist/js/jquery.justifiedGallery.min.js",
			function () {
